...
model = tf.keras.Sequential([
    tf.keras.layers.Conv2D(31, (2, 2), padding='same', activation=tf.nn.relu,
                           input_shape=(16, 16, 3)), # imput shape-ul este shapeul unei imagini 16*16 de tip rgb
    ...
    tf.keras.layers.Flatten(), #transformam imaginea intr-un numar
    tf.keras.layers.Dense(200, activation=tf.nn.relu),
    tf.keras.layers.Dense(100, activation=tf.nn.relu),
    tf.keras.layers.Dense(7,  activation=tf.nn.softmax) #densul final sa fie 7, deoarece avem 7 clase
])
...
model.fit(x=train_data, y=train_labels, epochs=30,batch_size = 16,verbose = 1) #impartim la 255, deoarece acuratetea creste cand avem numere in intervalul[0,1] in loc de [0,255]
